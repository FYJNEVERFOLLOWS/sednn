...
#  mkdir $WORKSPACE
  ...
  WORKSPACE="./wham_workspace"
  TR_SPEECH_DIR="./wham_data/train_speech"
  TR_MIXED_DIR="./wham_data/train_mixed"
  TE_SPEECH_DIR="./wham_data/test_speech"
  TE_MIXED_DIR="./wham_data/test_mixed"
  echo "Using full data. "
fi

# Create mixture csv. 
python prepare_data_for_wham.py create_mixture_csv --workspace=$WORKSPACE --speech_dir=$TR_SPEECH_DIR --mixed_dir=$TR_MIXED_DIR --data_type=train
python prepare_data_for_wham.py create_mixture_csv --workspace=$WORKSPACE --speech_dir=$TE_SPEECH_DIR --mixed_dir=$TE_MIXED_DIR --data_type=test

# Calculate mixture features.
TR_SNR=0
TE_SNR=0 
python prepare_data_for_wham.py calculate_mixture_features --workspace=$WORKSPACE --speech_dir=$TR_SPEECH_DIR --mixed_dir=$TR_MIXED_DIR --data_type=train --snr=$TR_SNR
python prepare_data_for_wham.py calculate_mixture_features --workspace=$WORKSPACE --speech_dir=$TE_SPEECH_DIR --mixed_dir=$TE_MIXED_DIR --data_type=test --snr=$TE_SNR

# Pack features. 
N_CONCAT=7
N_HOP=3
#python prepare_data_for_wham.py pack_features --workspace=$WORKSPACE --data_type=train --snr=$TR_SNR --n_concat=$N_CONCAT --n_hop=$N_HOP
#python prepare_data_for_wham.py pack_features --workspace=$WORKSPACE --data_type=test --snr=$TE_SNR --n_concat=$N_CONCAT --n_hop=$N_HOP

# Compute scaler. 
#python prepare_data_for_wham.py compute_scaler --workspace=$WORKSPACE --data_type=train --snr=$TR_SNR
